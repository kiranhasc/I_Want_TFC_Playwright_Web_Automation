...
test.describe('Continue Watching - No Watch History', () => {
    ...
    test('@Medium IW3-T1928: Verify the Continue Watching tray on the Home Page for new users upon watching less than 5% of the content', async ({ page }) => {
        test.setTimeout(120000);
        const data = testCaseData['tc-auth-029-continue-watching-less-than-five-percent'] as { mode?: string; searchTerm?: string } | undefined;
        const result = await verifyContinueWatchingTrayForNewUserAfterLessThanFivePercentPlayback(page, { mode: data?.mode, searchTerm: data?.searchTerm });
        expect(result.isValid).toBeTruthy();
    ...
    test('@Medium IW3-T1929: Verify the Continue Watching tray upon watching 50% of the content', async ({ page }) => {
        test.setTimeout(120000);
        const data = testCaseData['tc-auth-028-continue-watching-fifty-percent'] as { mode?: string; searchTerm?: string } | undefined;
        const result = await verifyContinueWatchingTrayAfterFiftyPercentPlayback(page, { mode: data?.mode, searchTerm: data?.searchTerm });
        expect(result.isValid).toBeTruthy();
        expect(result.trayVisible).toBeTruthy();
        expect(result.contentAppearsInTray).toBeTruthy();
    ...
    test('@Medium IW3-T1930: Verify that latest watched episode/movie of a season gets updated in the Continue Watching tray', async ({ page }) => {
        test.setTimeout(120000);
        const data = testCaseData['tc-cw-1930-latest-watched-episode'] as { mode?: string; searchTerm?: string; expectedSeasonNumber?: string; expectedEpisodeNumber?: string } | undefined;
    ...
    test('@Medium IW3-T1946: Verify that the season number and episode number are updated after completing the last episode of the current season and partially playing an episode from the next season', async ({ page }) => {
        ...
        const data = testCaseData['tc-cw-1946-season-episode-update'] as { mode?: string; query?: string; seasonOneEpisode?: string; seasonOneSeekPosition?: number; seasonTwoSeekPosition?: number; expectedSeasonNumber?: string; expectedEpisodeNumber?: string } | undefined;
    ...
    test('@Medium IW3-T1948: Verify that the Continue Watching tray is displayed with content of all types (free and premium)', async ({ page }) => {
        const data = testCaseData['tc-cw-1948-free-and-paid-content'] as { mode?: string } | undefined;
    ...
    test('@Medium IW3-T1947: Verify that the subscription prompt appears when attempting to play a premium episode from the Continue Watching tray', async ({ page }) => {
        ...
        const data = testCaseData['tc-cw-1947-continue-watching-subscription-prompt'] as { mode?: string } | undefined;
    ...
    test('@Medium IW3-T1942: Verify that Ad gets played for the free user on resuming the content', async ({ page }) => {
        test.setTimeout(120000);
        const data = testCaseData['tc-cw-1942-free-user-ad-resume'] as { mode?: string } | undefined;
